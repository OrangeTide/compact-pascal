...
## 1: Operator Overloading - from Vector Pascal
  ...
  **Commutativity is explicit.** An overload is keyed on `(symbol, lhs-type, rhs-type)`. If a user wants both `complex * real` and `real * complex`, they declare both — typically pointing at the same function with swapped argument order. No implicit reverse lookup, no Python-style `__rmul__` fallback. This avoids the Wren asymmetry bug (where binary operators dispatch only on the left operand and `1.0 / complex` can't be expressed) without introducing hidden semantics. The cost is verbosity; the payoff is that operator dispatch is a straight symbol-table lookup with no second-chance mechanism, which matches the single-pass compiler and Pascal's character.
 
  **Built-in numeric promotion before lookup.** When searching for a matching overload, the compiler applies the same `integer → real` promotion it uses for built-in operators before checking the overload table. So `operator * (real, complex)` is found by `int_val * complex` — the integer is promoted to real first, then the overload matches. This halves the combinatorial pressure on library authors (`integer`/`real` pairs collapse to one declaration) without introducing a new language concept. The rule is: same promotion as built-ins, nothing more. User types do not chain — `MyInt → integer → real → complex` is not attempted; only built-in numeric promotions apply. Richer coercion (user-defined implicit conversions, Delphi-style `operator :=`) is a separate proposal deferred until `real` lands and the real-world combinatorial pressure is measurable. The guiding principle: the language is meant to be implementable by a normal person, and a single pre-lookup promotion step is; a general coercion search is not.
